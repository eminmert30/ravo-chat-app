...
export async function POST(req: Request) {
  try {
    ...
    // --- Socket.IO ile anlık eventler ---
    // @ts-ignore
    const io = global.io;
    if (io) {
      io.to(request.senderId).emit("refreshFriendsList");
      io.to(request.receiverId).emit("refreshFriendsList");
      io.to(request.senderId).emit("newChat", { type: "private" });
      io.to(request.receiverId).emit("newChat", { type: "private" });
    }
